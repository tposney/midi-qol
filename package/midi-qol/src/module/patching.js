import { log, i18n, getCanvas } from "../midi-qol.js";
...
async function bonusCheck(actor, result, checkName) {
	if (!installedModules.get("betterrolls5e")) {
		const bonusFlags = Object.keys(actor.data.flags["midi-qol"]?.optional ?? [])
			.filter(flag => {
			if (!actor.data.flags["midi-qol"].optional[flag][checkName])
				return false;
...
async function rollAbilityTest(wrapped, ...args) {
	...
	if (chatMessage !== false && result) {
		...
		if (game.system.id === "sw5e")
			setProperty(args, "flags.sw5e.roll", { type: "ability", abilityId });
		result.toMessage(args);
	}
...
async function rollAbilitySave(wrapped, ...args) {
	...
	if (chatMessage !== false && result) {
		...
		if (game.system.id === "sw5e")
			setProperty(args, "flags.sw5e.roll", { type: "save", abilityId });
		result.toMessage(args);
	}
...
function __midiATIRefresh(template) {
	...
	if (installedModules.get("levelsvolumetrictemplates")) {
		...
		const tokensToCheck = canvas.tokens.placeables?.filter(tk => {
			...
			//@ts-ignore
			{ x: tk.x, y: tk.y, z: tk.data.elevation }, { x: template.x, y: template.y, z: template.data.flags.levels?.elevation ?? 0 }, "sight")) {
				return false;
			}
			return true;
		});
