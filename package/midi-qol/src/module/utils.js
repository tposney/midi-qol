import { debug, i18n, error, warn, noDamageSaves, cleanSpellName, MQdefaultDamageType, allAttackTypes, gameStats, debugEnabled, getCanvas, overTimeEffectsToDelete } from "../midi-qol.js";
	...
	if (configSettings.damageImmunities !== "none" && dmgTypeString !== "") {
		...
		for (let { type, mult } of [{ type: "di", mult: 0 }, { type: "dr", mult: 0.5 }, { type: "dv", mult: 2 }]) {
			...
			if (!(magicalDamage || silverDamage) && trait.includes("silver"))
				trait = trait.concat("bludgeoning", "slashing", "piercing");
			if (item?.type === "spell" && trait.includes("spell") && !["healing", "temphp"].includes(dmgTypeString))
...
export let applyTokenDamage = async (damageDetail, totalDamage, theTargets, item, saves, options = { existingDamage: [], superSavers: new Set() }) => {
...
export let applyTokenDamageMany = async (damageDetailArr, totalDamageArr, theTargets, item, savesArr, options = { existingDamage: [], superSavers: [] }) => {
	let damageList = [];
	let targetNames = [];
	let appliedDamage;
...
export async function processDamageRoll(workflow, defaultDamageType) {
	...
	if (["rwak", "mwak"].includes(item?.data.data.actionType) && configSettings.rollOtherDamage) {
		if (workflow.otherDamageRoll && configSettings.singleConcentrationRoll) {
			appliedDamage = await applyTokenDamageMany([workflow.damageDetail, workflow.otherDamageDetail, workflow.bonusDamageDetail ?? []], [workflow.damageTotal, workflow.otherDamageTotal, workflow.bonusDamageTotal ?? 0], theTargets, item, [new Set(), workflow.saves, new Set()], {
				existingDamage: [],
			...
			appliedDamage = await applyTokenDamageMany([workflow.damageDetail, workflow.bonusDamageDetail ?? []], [workflow.damageTotal, workflow.bonusDamageTotal ?? 0], theTargets, item, [savesToUse, savesToUse], {
				existingDamage: [],
		...
		appliedDamage = await applyTokenDamageMany([workflow.damageDetail, workflow.bonusDamageDetail ?? []], [workflow.damageTotal, workflow.bonusDamageTotal ?? 0], theTargets, item, [workflow.saves, workflow.saves], {
			existingDamage: [],
			superSavers: [workflow.superSavers, workflow.superSavers]
		});
	}
...
export let getSaveMultiplierForItem = (item) => {
	...
	if (description.includes(i18n("midi-qol.fullDamage").toLocaleLowerCase()))
		return 1;
					...
					for (let change of changes) {
						...
						if ((endTurn && changeTurnEnd) || (startTurn && changeTurnStart)) {
							...
							try {
								...
								if (!rollPromise)
									rollPromise = completeItemRoll(ownedItem, options);
								else
									rollPromise = rollPromise.then((data) => completeItemRoll(ownedItem, options));
							}
							finally {
...
export function isFastForwardSpells(workFlow = undefined) {
...
export async function processAttackRollBonusFlags() {
	...
	const bonusFlags = Object.keys(this.actor.data.flags["midi-qol"]?.optional ?? [])
		.filter(flag => {
		if (!this.actor.data.flags["midi-qol"].optional[flag].attack)
			return false;
...
export async function processDamageRollBonusFlags() {
	const bonusFlags = Object.keys(this.actor.data.flags["midi-qol"]?.optional ?? [])
		.filter(flag => {
		if (!this.actor.data.flags["midi-qol"].optional[flag].damage)
			return false;
		if (!this.actor.data.flags["midi-qol"].optional[flag].count)
			return true;
		return getOptionalCountRemainingShortFlag(this.actor, flag) > 0;
...
export async function doReactions(target, triggerTokenUuid, attackRoll, triggerType) {
	const noResult = { name: undefined, uuid: undefined, ac: undefined };
	if (!target.actor || !target.actor.data.flags || !target.actor.data.flags["midi-qol"])
		return noResult;
	...
	if (getReactionSetting(player) === "none")
		return noResult;
	if (target.actor.getFlag("midi-qol", "reactionCombatRound"))
		return noResult;
	let reactions = target.actor.items.filter(item => {
		...
		if ((item.data.data.activation.condition ?? "" !== "") && !item.data.data.activation.condition.includes(triggerType))
			return false;
		//@ts-ignore
		if (triggerType === i18n("midi-qol.damaged") && (item.data.data.activation.condition ?? "") === "")
			return false;
...
export async function requestReactions(target, player, triggerTokenUuid, attackRoll, triggerType, resolve) {
	if (!attackRoll)
		attackRoll = await new Roll("0").roll();
...
export async function promptReactions(tokenUuid, triggerTokenUuid, attackRoll, triggerType) {
...
export async function reactionDialog(actor, triggerTokenUuid, reactionItems, attackRoll, triggerType) {
	return new Promise((resolve, reject) => {
		const callback = async (dialog, button) => {
		...
		let content;
		if (triggerType === i18n("midi-qol.damaged"))
			content = i18n("midi-qol.Damage");
		else
...
export function evalActivationCondition(workflow, condition) {
