import { debug, warn, i18n, MESSAGETYPES, gameStats, debugEnabled, MQdefaultDamageType } from "../midi-qol.js";
		...
		if (entry.type === "damage-group") {
			for (const subEntry of entry.entries) {
				...
				if (type === "") {
					...
					if (item?.data.data.actionType === "heal")
						type = "healing";
				}
	...
	if (["other", "damage"].includes(message.data.flags?.dnd5e?.roll?.type)) {
		...
		if (message.data.flags?.dnd5e?.roll?.type === "damage") {
			...
			if (game.system.id === "sw5e" && !itemId)
				itemId = message.data.flags.sw5e?.roll.itemId;
