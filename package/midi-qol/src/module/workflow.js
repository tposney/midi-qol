//@ts-ignore
import { warn, debug, log, i18n, MESSAGETYPES, error, MQdefaultDamageType, debugEnabled, timelog, getCanvas } from "../midi-qol.js";
...
export class Workflow {
	constructor(actor, item, speaker, targets, options) {
		...
		if (installedModules.get("levels") && token) {
			...
			this.templateElevation = 0;
		if (installedModules.get("levels")) {
			if (game.user)
				setProperty(game.user, "data.flags.midi-qol.elevation", this.templateElevation);
		}
	...
	async _next(newState, undoData = {}) {
		...
		switch (newState) {
			case WORKFLOWSTATES.NONE:
				this.preSelectedTargets = new Set(game.user?.targets); // record those target selected before cast.
			...
			case WORKFLOWSTATES.PREAMBLECOMPLETE:
				this.effectsAlreadyExpired = [];
				if (Hooks.call("midi-qol.preambleComplete", this) === false)
					return;
			...
			case WORKFLOWSTATES.ATTACKROLLCOMPLETE:
				...
				if ((getAutoRollDamage() === "onHit" && this.hitTargetsEC.size === 0 && this.hitTargets.size === 0 && this.targets.size !== 0)
			...
			case WORKFLOWSTATES.DAMAGEROLLCOMPLETE:
				...
				if (this.item?.data.data.actionType === "heal" && !Object.keys(CONFIG.DND5E.healingTypes).includes(this.defaultDamageType))
			...
			case WORKFLOWSTATES.APPLYDYNAMICEFFECTS:
				...
				if (configSettings.allowUseMacro && !this.onUseMacroCalled && this.item?.data.flags) {
					...
					const results = await this.callMacros(this.item, getProperty(this.item, "data.flags.midi-qol.onUseMacroName"), "OnUse");
					// Special check for return of {haltEffectsApplication: true} from item macro
					if (results.some(r => r?.haltEffectsApplication))
						return this.next(WORKFLOWSTATES.ROLLFINISHED);
				}
				if (Hooks.call("midi-qol.preApplyDynamicEffects", this) === false)
					return this.this.next(WORKFLOWSTATES.ROLLFINISHED);
				...
				if (!this.item)
					return this.next(WORKFLOWSTATES.ROLLFINISHED);
				if (!configSettings.autoItemEffects && !this.forceApplyEffects)
			...
			case WORKFLOWSTATES.ROLLFINISHED:
				...
				if (configSettings.allowUseMacro && !this.onUseMacroCalled) {
					...
					await this.callMacros(this.item, getProperty(this.item, "data.flags.midi-qol.onUseMacroName"), "OnUse");
				}
	...
	*/
	async checkSaves(whisper = false) {
		...
		try {
			...
			for (let target of allHitTargets) {
				...
				if ((!player?.isGM && playerMonksTB) || (player?.isGM && gmMonksTB)) {
					...
					//@ts-ignore CONFIG.DND5E
					if (debugEnabled > 0)
						warn(`Player ${player?.name} controls actor ${target.actor.name} - requesting ${CONFIG.DND5E.abilities[this.saveItem.data.data.save.ability]} save`);
					promises.push(new Promise((resolve) => {
		...
		if (rollType === "save")
			//@ts-ignore CONFIG.DND5E
			this.saveDisplayFlavor = `${this.item.name} <label class="midi-qol-saveDC">DC ${rollDC}</label> ${CONFIG.DND5E.abilities[rollAbility]} ${i18n(allHitTargets.size > 1 ? "midi-qol.saving-throws" : "midi-qol.saving-throw")}:`;
		else if (rollType === "check")
			//@ts-ignore CONFIG.DND5E
			this.saveDisplayFlavor = `${this.item.name} <label class="midi-qol-saveDC">DC ${rollDC}</label> ${CONFIG.DND5E.abilities[rollAbility]} ${i18n(AllHitTargets.size > 1 ? "midi-qol.ability-checks" : "midi-qol.ability-check")}:`;
	...
	processDefenceRoll(message, html, data) {
		...
		if (!isLMRTFY || message.data.flags?.dnd5e?.roll?.type === "save")
			return true;
	...
	async checkHits() {
		...
		for (let targetToken of this.targets) {
			...
			if (this.useActiveDefence) {
				...
				if (!this.isFumble) {
					...
					if ((isHit || isHitEC || this.iscritical) && this.attackRoll) {
						...
						if (targetEC)
							targetEC = targetActor.data.data.attributes.ac.EC + bonusAC;
						if (result.ac)
				...
				if (checkRule("challengeModeArmorScale") && !this.isCritical)
					scale = Math.floor((this.attackTotal - targetEC + 1) / ((targetActor?.data.data.attributes.ac.AR ?? 0) + 1) * 10) / 10;
				setProperty(targetToken.actor?.data ?? {}, "flags.midi-qol.challengeModeScale", scale);
				if (this.isCritical)
					isHit = true;
				if (isHit || isHitEC || this.isCritical)
					this.processCriticalFlags();
			...
			let hitScale = 100;
			if (checkRule("challengeModeArmorScale") && !this.isCritical)
				hitScale = Math.floor((getProperty(targetToken.actor?.data ?? {}, "flags.midi-qol.challengeModeScale") ?? 1) * 100);
			let hitString;
			if (this.isCritical)
	...
	async checkActiveAttacks(attackBonus = 0, whisper = false, fumbleTarget, criticalTarget) {
		...
		for (let target of this.targets) {
			...
			if (!this.rollOptions.advantaage && this.rollOptions.disadvantage)
		...
		for (let target of this.targets) {
			...
			if (game.user?.isGM)
				log(`Ability active defemce: ${target.name} rolled ${rollTotal} vs attack DC ${this.activeDefenceDC}`);
			i++;
		}
	}
}
	...
	async _next(newState) {
		...
		switch (newState) {
			...
			case WORKFLOWSTATES.DAMAGEROLLCOMPLETE:
				...
				return this.next(WORKFLOWSTATES.ALLROLLSCOMPLETE);
			case WORKFLOWSTATES.ROLLFINISHED:
				if (this.placeTemlateHookId)
					Hooks.off("createMeasuredTemplate", this.placeTemlateHookId);
				await this.complete();
