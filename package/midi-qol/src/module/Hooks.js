...
export let readyHooks = async () => {
	...
	Hooks.on("preUpdateActor", (actor, update, diff, user) => {
		...
		if (hpUpdate >= (actor.data.data.attributes.hp.tempmax ?? 0) + actor.data.data.attributes.hp.max)
			hpDiff = 0;
	...
	Hooks.on("updateToken", (tokenDocument, update, diff, userId) => {
		...
		if (expiredEffects.length > 0)
			actor?.deleteEmbeddedDocuments("ActiveEffect", expiredEffects.map(ef => ef.id));
	});
...
export function restManager(actor, result) {
	...
	if (myExpiredEffects?.length > 0)
		actor?.deleteEmbeddedDocuments("ActiveEffect", myExpiredEffects);
}
...
export function initHooks() {
	...
	Hooks.on('dropCanvasData', function (canvas, dropData) {
		...
		if (!actor || !item)
			error("actor / item broke ", actor, item);
		//@ts-ignore roll
		item?.roll();
		return true;
	});
