...
function _isVisionSource() {
    ...
    if (this.data.hidden && !(isGM || this.actor?.testUserPermission(game.user, "OWNER")))
        return false;
    ...
    if (isGM)
        return false;
    if (this.actor?.testUserPermission(game.user, "OBSERVER"))
        return true;
...
function isVisible() {
    ...
    if (this.data.hidden)
        return gm || this.actor?.testUserPermission(game.user, "OWNER");
