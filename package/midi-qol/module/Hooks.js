...
export let readyHooks = async () => {
    ...
    Hooks.on("preUpdateActor", (actor, update, diff) => {
        ...
        if (hpUpdate === undefined)
            return true;
        if (game.cub && !game.cub?.hasCondition(concentrationName, actor))
            return true;
        let concentrationEffect = !game.cub && actor.effects.find(e => e.data.label === concentrationName);
        if (!game.cub && !concentrationEffect)
            return;
