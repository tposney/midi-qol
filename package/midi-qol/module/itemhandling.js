import { warn, debug, error, i18n, MESSAGETYPES, i18nFormat, gameStats } from "../midi-qol.js";
...
import { addConcentration, checkRange, getAutoRollAttack, getAutoRollDamage, getRemoveDamageButtons, getSelfTarget, getSelfTargetSet, isAutoFastAttack, isAutoFastDamage, itemHasDamage, itemIsVersatile, validTargetTokens } from "./utils.js";
...
export async function doAttackRoll(wrapped, options = { event: { shiftKey: false, altKey: false, ctrlKey: false, metaKey: false }, versatile: false, resetAdvantage: false, chatMessage: false }) {
    ...
    if (workflow.workflowType === "BetterRollsWorkflow")
        displayChat = options.chatMessage;
...
export async function doDamageRoll(wrapped, { event = {}, spellLevel = null, powerLevel = null, versatile = null, options = {} } = {}) {
    ...
    if (configSettings.rollOtherDamage &&
...
export async function doItemRoll(wrapped, options = { showFullCard: false, createWorkflow: true, versatile: false, configureDialog: true, event }) {
    ...
    workflow.showCard = configSettings.mergeCard || (configSettings.showItemDetails !== "none") || ((this.isHealing && getAutoRollDamage() === "none") || // not rolling damage
    ...
    if (workflow.showCard) { //TODO check this against dnd5e 1.30
        ...
        var itemCard = await showItemCard.bind(item)(showFullCard, workflow);
...
export async function showItemCard(showFullCard, workflow, minimalCard = false) {
    ...
    if (isAutoFastDamage())
        versaBtnText += `${i18n("DND5E.Versatile")} ${i18n("midi-qol.fastForward")}`;
    const templateData = {
        actor: this.actor,
        tokenId: token?.uuid || null,
        item: this.data,
...
function isTokenInside(token, wallsBlockTargeting) {
    const grid = canvas.scene.data.grid, templatePos = { x: this.data.x, y: this.data.y };
