//@ts-ignore
import { warn, debug, log, i18n, MESSAGETYPES, error, MQdefaultDamageType, timelog } from "../midi-qol.js";
...
export class Workflow {
                ...
                for (let target of this.targets) {
                    ...
                    const expiredEffects = target.actor?.effects?.filter(ef => {
                        ...
                        if ((specialDuration.includes("isAttacked") && wasAttacked) ||
                            ...
                            return true;
                        if (this.item.hasSave && specialDuration.includes(`isSaveSuccess`) && this.saves.has(target))
                            return true;
                        if (this.item.hasSave && specialDuration.includes(`isSaveFailure`) && !this.saves.has(target))
                            return true;
                        const abl = this.item.data.data.save.ability;
                        if (this.item.hasSave && specialDuration.includes(`isSaveSuccsss.${abl}`) && this.saves.has(target))
                            return true;
                        if (this.item.hasSave && specialDuration.includes(`isSaveFailure.${abl}`) && !this.saves.has(target))
                            return true;
                        for (let dt of this.damageDetail) {
                            if (specialDuration.includes(`isDamaged.${dt.type}`))
                                return true;
                        }
                        return false;
                    }).map(ef => ef.id);
    ...
    async callMacro(macroName, macroData) {
            ...
            if (this.attackRoll.dice.length) {
                ...
                if (isD20) {
                    ...
                    if (d.options.critical && (d.total >= d.options.critical)) {
                        ...
                        if (this.attackRoll.total >= d.options.target)
    ...
    async checkSaves(whisper = false) {
                ...
                if ((!player.isGM && playerMonksTB) || (player.isGM && gmMondsTB)) {
                    ...
                    promises.push(new Promise((resolve) => {
                        ...
                        if (["letme", "letmeQuery"].includes(configSettings.playerRollSaves) && installedModules.get("lmrtfy"))
                            requestId = randomID();
                        if (["letme", "letmeQuery"].includes(configSettings.rollNPCSaves) && installedModules.get("lmrtfy"))
                            requestId = randomID();
                        this.saveRequests[requestId] = resolve;
        ...
        if (monkRequests.length > 0) {
            game.MonksTokenBar.requestRoll(monkRequests, {
