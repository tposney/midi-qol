import { debug, warn, i18n, MESSAGETYPES, gameStats } from "../midi-qol.js";
...
import { createDamageList, getTraitMult, calculateDamage, addConcentration, MQfromUuid, getSelfTarget } from "./utils.js";
...
export let processpreCreateBetterRollsMessage = (message, data, options, user) => {
    ...
    if (!workflow)
        workflow = new BetterRollsWorkflow(actor, item, speaker, targets, null);
...
export let diceSoNiceHandler = async (message, html, data) => {
    if (!game.dice3d || !installedModules.get("dice-so-nice") || game.dice3d.messageHookDisabled || !game.dice3d.isEnabled())
        return;
    ...
    if (!getProperty(message.data, "flags.midi-qol.waitForDiceSoNice"))
        return;
...
export let nsaMessageHandler = (message, data, ...args) => {
    if (!nsaFlag || !data.whisper || data.whisper.length === 0)
        return true;
